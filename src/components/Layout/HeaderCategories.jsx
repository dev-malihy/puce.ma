  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      for (let cat of cateData) {
        ...
        if (
          totalWidth + catWidth + (count > 0 ? gap : 0) + otherCategoryWidth <=
          containerWidth
        ) {
            ...
            <div className="shopping_items" ref={containerRef}>
              {cateData.slice(0, fitCategoriesCount).map((cat) => {
                return cat.subcategories_count > 0 ? (
                  <div
                    className={`shopping_cat ${
                      headerCatSelected === cat?.slug && "brdrShop"
                    }`}
                    onMouseEnter={() => handleCatClick(cat)}
                    key={cat?.id}
                    onMouseLeave={() => setIsShowCatDrop(false)}
                  >
                    <span>{cat?.translated_name}</span>
                    {cat?.translated_name === MenuData?.translated_name &&
                    IsShowCatDrop ? (
                      <span>
                        <FaAngleUp className="prof_down_arrow" />
                      </span>
                    ) : (
                      <span>
                  ...
                  <Link
                    ...
                    key={cat?.id}
                    onMouseEnter={handleCatLinkClick}
                  >
                    {cat?.translated_name}
                  </Link>
                );
              ...
              {cateData && cateData.length > fitCategoriesCount && (
                <div
                  className={`shopping_cat ${
                    IsShowOtherCat ||
                    (headerCatSelected === "products" && "brdrShop")
                  }`}
                  onMouseLeave={() => setIsShowOtherCat(false)}
                  onMouseEnter={handleOtherCat}
                >
                  <span>{t("other")}</span>
                  {IsShowOtherCat ? (
                    <span>
                    ...
                    <div className="selected_cat">
                      <div className="cat_link_cont">
                        <Image
                          ...
                          height={22}
                        />
                        <span>{MenuData?.translated_name}</span>
                      </div>
                    </div>
                    <div className="allCatWrapper">
                      {MenuData?.subcategories_count > 0 && (
                        <>
                          ...
                          >
                            {t("seeAllIn")} {MenuData?.translated_name}
                          </Link>
                          {MenuData?.subcategories.map((sub) => (
                            <div
                              className="cate_item cate_subcate"
                              key={sub?.slug}
                            >
                              <Link
                                href={`/category/${sub?.slug}`}
                                className="main_cat"
                                onClick={() => setIsShowCatDrop(false)}
                              >
                                {sub?.translated_name}
                              </Link>
                              {sub?.subcategories_count > 0 &&
                                sub.subcategories
                                  ...
                                  .map((nestedSub) => (
                                    <Link
                                      onClick={() => setIsShowCatDrop(false)}
                                      href={`/category/${nestedSub?.slug}`}
                                      className="subcat"
                                      key={nestedSub?.slug}
                                    >
                                      {nestedSub?.translated_name}
                                    </Link>
                                  ))}
              ...
              {IsShowOtherCat && (
                <div className="cate_cont_wrap">
                  ...
                  >
                    ...
                    <div className="allCatWrapper">
                      <Link
                        onClick={() => setIsShowOtherCat(false)}
                        href="/products"
                        className="see_all_cat"
                      >
                        {t("seeAllIn")} {t("other")}
                      ...
                      {cateData &&
                        cateData.slice(fitCategoriesCount).map((sub) => (
                          <div
                            className="cate_item cate_subcate"
                            key={sub?.slug}
                          >
