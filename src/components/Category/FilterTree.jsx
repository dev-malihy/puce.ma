...
const FilterTree = ({ slug, show, setShow, setCategoryIds }) => {
  ...
  const initialSelectedKeys =
    pathname === "/products" ? ["all-categories"] : slug ? [slug] : [];
  ...
  useEffect(() => {
    const fetchDataAndSetTree = async () => {
      if (
        ...
        const tree = constructTreeData(cateData, [
          {
            name: t("allCategories"),
            engName: t("allCategories"),
            key: "all-categories",
            slug: "/products",
          },
        ]);
        const totalApprovedItems =
          tree?.length > 0 ? tree?.reduce((sum, cat) => sum + cat.count, 0) : 0;
        dispatch(
          setTreeData([
            {
  ...
  const updateCategoryNamesWithTranslation = (treeData, CurrentLanguage) => {
    return treeData.map((node) => {
      ...
      const updatedPath = node?.path?.map((pathItem) => {
        ...
        const translatedName = pathItemTranslation
          ? pathItemTranslation.name
          : pathItem.engName;
        const updatedPathItem = {
          ...pathItem,
          name:
            pathItem.key === "all-categories"
              ? t("allCategories")
              : translatedName, // Set translated name for 'all-categories'
        };
        return updatedPathItem;
      });
      const nodaName =
        node.key === "all-categories"
          ? t("allCategories")
          : nodeTranslation
          ? nodeTranslation.name
          : node.name;
  ...
  useEffect(() => {
    if (treeData.length > 0) {
      ...
      dispatch(
        setTreeData(
          updateCategoryNamesWithTranslation(treeData, CurrentLanguage)
        )
      );
    }
  }, [CurrentLanguage]);
  ...
  const renderTreeNode = (node) => {
    ...
    return (
      <div
        className={`filter_item_cont ${
          isSelected || isAllCatSelected ? "selected" : ""
        }`}
      >
        {node.isLoadMore ? (
          IsLoadMore ? (
            ...
            <span
              ...
              onClick={() =>
                fetchMoreSubcategory(
                  node?.id,
                  node.path,
                  node?.getMoreKey,
                  subCategoryPages[node.getMoreKey]?.currentPage + 1
                )
              }
            >
              {node?.title}
          ...
          <>
            <span className={`filter_item ${isBold ? "bold" : ""}`}>
              {node?.title}
            </span>
            <span className={`filter_item_count ${isBold ? "bold" : ""}`}>
              ({node.count})
            </span>
          </>
        )}
  ...
  const switcherIcon = (node) => {
    ...
    return node?.expanded ? (
      <LuMinus size={14} color={iconColor} fontWeight={600} />
    ) : (
      <GoPlus size={14} color={iconColor} fontWeight={600} />
    );
  };
  ...
  const fetchMoreCategory = async () => {
    ...
    try {
      ...
      const newTreeData = await constructTreeData(data.data, [
        {
          name: t("allCategories"),
          engName: t("allCategories"),
          key: "all-categories",
